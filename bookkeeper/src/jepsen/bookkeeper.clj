  ...
  (reify db/DB
    (setup! [_ test node]
      ...
      (c/su
       ...
       (c/upload "resources/bookkeeper-server-4.4.0-bin.tar.gz" "/bookkeeper.tar.gz")
       ...
       ; Install zookeeper, moved into os below
  ...
  ([zk-addr zk-root-path timeout throttle]
   ...
   (try
    ...
    (catch org.apache.zookeeper.KeeperException$NoNodeException nne
      (throw (ex-info "Error locating BookKeeper cluster via ledger path. Check that BookKeeper has been started via start-env by setting `:bookkeeper/server? true` in env-config, or is setup at the correct path."
...
(defn basic-test
  ...
  [{:keys [awake-ms stopped-ms time-limit]}]
